...
*/
 ...
 (function($) {
    ...
    var ajax_stream = $.ajax = function(options) {
        ...
        options = jQuery.extend(true, options, jQuery.extend(true, {},
        jQuery.ajaxSettings, options));
        if (options.stream) {
            var timer = 0;
            var offset = 0;
            var xmlhttp = null;
            ...
            var check = function(status)
            {
                ...
                //if (xmlhttp.status < 3) return;
                //only get the latest packet if data has been sent
                var text = xmlhttp.responseText;
                if (status == 'stream')
                //if we arent streaming then just flush the buffer
                {
